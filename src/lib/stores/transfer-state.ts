...
import type { moneyWithTransferDetailsSchema } from "../server/fn/money"
import type { Database } from "../server/supabase/types"

export type MoneyWithTransferDetails =
	Database["public"]["Tables"]["money"]["Row"] & {
		fee?: number
		reason?: string
		cashIn?: number
	}
export type TransferState = {
	...
	selectForTransfer: (
		money: z.infer<typeof moneyWithTransferDetailsSchema>
	) => void
	cancel: () => void
	setReceiverData: (
		money: z.infer<typeof moneyWithTransferDetailsSchema>
...
export const useTransferState = create<TransferState>()(set => ({
	...
	setReceiverData: money =>
		set(state => {
			if (state.receivers?.find(r => r.id === money.id)) {
				return {
